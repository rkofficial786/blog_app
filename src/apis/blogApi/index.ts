...
const blogApi = {
  async getBlogs(filters = {}) {
    ...
    if (filters.language) {
      filteredBlogs = filteredBlogs.filter(blog => 
        blog.language === filters.language
      );
    }
  ...
  async createBlog(payload: Partial<BlogPost>) {
    const newBlog = {
      id: `blog-${Date.now()}`,
      ...payload,
      likes: 0,
    ...
    const enrichedBlog = enrichBlogWithAuthor(newBlog as BlogPost);
  ...
  async getBlogsByAuthor(authorId: string) {
    const authorBlogs = mockDataStore.blogs.filter(blog => blog.authorId === authorId);
